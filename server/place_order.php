...
if(isset($_POST['place_order'])){
    ...
    $stmt = $conn -> prepare("INSERT INTO orders (order_cost, order_status, user_id, user_phone, user_city, user_address, order_date) 
                        VALUES (?,?,?,?,?,?,?);");
                        //For better protection (protect system from hackers)
    ...
    //Issue new roder and store order info in database 
    $order_id = $stmt -> insert_id;

    //Get products from cart (from session)
    foreach($_SESSION['cart'] as $key => $value) // [ 4=>[] , 5=>[] ]
    {
        ...
        //Store each single item in order_item database
    ...
    //Inform user wheter everything is fine or there is a problem
